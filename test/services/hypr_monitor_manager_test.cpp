...
Monitor makeTestMonitor(const std::string &name = "HDMI-A-1", int width = 1920,
                        int height = 1080) {
  Monitor m;
  m.setName(name);
  m.setWidth(width);
...
TEST(HyprMonitorManagerTest,
     ApplyMonitorConfigurationGeneratesCommandForEnabledMonitor) {
  ...
  EXPECT_THAT(lastCommand, ::testing::HasSubstr(
                               "hyprctl keyword monitor DP-1,1920x1080@75.50"));
...
TEST(HyprMonitorManagerTest,
     ApplyMonitorConfigurationGeneratesCommandForDisabledMonitor) {
  std::shared_ptr<MockParser> parser;
  HyprMonitorManager manager(parser);
  ...
  EXPECT_TRUE(result);
  EXPECT_EQ(systemCallCount, 1);
  EXPECT_THAT(lastCommand,
              ::testing::HasSubstr("hyprctl keyword monitor HDMI-A-1,disable"));
}
...
TEST(HyprMonitorManagerTest, RevertMonitorConfigurationUsesBackup) {
  ...
  std::vector<Monitor> backupMonitors = {makeTestMonitor("DP-1", 1920, 1080)};

  EXPECT_CALL(*parser, parseMonitorsFromJson(_))
      .WillOnce(Return(backupMonitors));
