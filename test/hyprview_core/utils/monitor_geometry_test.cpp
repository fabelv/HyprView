...
TEST(MonitorGeometryTest, SnapReturnsOriginalIfAlone) {
    ...
    Position result = MonitorGeometry::getSnappedPosition(m, all, m.getPositionX(), m.getPositionY());
...
TEST(MonitorGeometryTest, SnapsToRightSide) {
    ...
    Position snapped = MonitorGeometry::getSnappedPosition(right, monitors, right.getPositionX(), right.getPositionY());
...
TEST(MonitorGeometryCornerSnapTest, SnapsToTopLeftCorner) {
    ...
    Position result = MonitorGeometry::getSnappedPosition(dragged, monitors, dragged.getPositionX(), dragged.getPositionY());
...
TEST(MonitorGeometryCornerSnapTest, SnapsToBottomRightCorner) {
    Monitor dragged = makeMonitor(-90, -90, 100, 100);  // slightly off from (10,10)
...
TEST(MonitorGeometryCornerSnapTest, SnapsToTopRightCorner) {
    Monitor dragged = makeMonitor(-90, 90, 100, 100);  // slightly off from (10,100)
...
TEST(MonitorGeometryTest, SnapsToBottomSide) {
    ...
    Position snapped = MonitorGeometry::getSnappedPosition(bottom, monitors, bottom.getPositionX(), bottom.getPositionY());
...
TEST(MonitorGeometryTest, CalculatesPreviewScaleFactor) {
    ...
    double scale =
        MonitorGeometry::calculatePreviewScaleFactor(previewWidth, previewHeight, margin, monitors);
...
TEST(MonitorGeometryTest, CalculatesCenteredOffsetCorrectly) {
    ...
    Position offset =
        MonitorGeometry::calculateCenteredOffset(monitors, scale, areaWidth, areaHeight);
