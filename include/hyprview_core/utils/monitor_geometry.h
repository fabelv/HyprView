...
public:
  ...
  static Position
  calculateCenteredOffset(const std::vector<Monitor> &allMonitors,
                          double scaleFactor, int areaWidth, int areaHeight);
  static double
  calculatePreviewScaleFactor(int areaWidth, int areaHeight,
                              float marginPercentage,
                              const std::vector<Monitor> &allMonitors);

private:
  static Bounds computeBoundsOffset(const std::vector<Monitor> &allMonitors);
  static std::pair<int, int> findClosestSnap(const Monitor &dragged,
                                             const Monitor &other,
                                             int &bestDistance);
};
