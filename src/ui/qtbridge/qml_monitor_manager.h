...
class QmlMonitorManager : public QObject {
  ...
  // qml properties
  Q_PROPERTY(
      QList<QmlMonitor *> monitors_ READ getMonitors NOTIFY monitorsChanged)
  Q_PROPERTY(int selectedMonitorIndex_ READ getSelectedMonitorIndex WRITE
                 setSelectedMonitorIndex NOTIFY selectedMonitorIndexChanged)

public:
  explicit QmlMonitorManager(core::HyprMonitorManager *coreManager,
  ...
  Q_INVOKABLE QPoint calculateOffsetToCenter(double scaleFactor, int width,
                                             int height);
