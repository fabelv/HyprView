...
Position
MonitorGeometry::getSnappedPosition(const Monitor &dragged,
  ...
  log(LogLevel::Debug, "Snapped to X=" + std::to_string(finalX) +
                           " Y=" + std::to_string(finalY));
  return Position(finalX, finalY);
}

std::pair<int, int> MonitorGeometry::findClosestSnap(const Monitor &dragged,
                                                     const Monitor &other,
                                                     int &bestDistance) {
  int dx = 0;
  int dy = 0;
...
double MonitorGeometry::calculatePreviewScaleFactor(
    int areaWidth, int areaHeight, float marginPercent,
    const std::vector<Monitor> &allMonitors) {
...
Position MonitorGeometry::calculateCenteredOffset(
    const std::vector<Monitor> &allMonitors, double scaleFactor, int areaWidth,
    int areaHeight) {
  auto [minX, minY, maxX, maxY] = computeBoundsOffset(allMonitors);
  log(LogLevel::Debug,
      "minX" + std::to_string(minX) + " minY" + std::to_string(minY));
  log(LogLevel::Debug,
      "maxX" + std::to_string(maxX) + " maxY" + std::to_string(maxY));

  int scaledWidth = static_cast<int>((maxX - minX) * scaleFactor);
  int scaledHeight = static_cast<int>((maxY - minY) * scaleFactor);
  log(LogLevel::Debug, "scaledWidth" + std::to_string(scaledWidth) +
...
Bounds
MonitorGeometry::computeBoundsOffset(const std::vector<Monitor> &allMonitors) {
  int minX = INT_MAX;
  int minY = INT_MAX;
  int maxX = INT_MIN;
  int maxY = INT_MIN;
