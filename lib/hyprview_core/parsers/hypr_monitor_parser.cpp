...
auto HyprMonitorParser::parseSingleMonitor(const nlohmann::json &obj)
    -> std::optional<Monitor> {
  try {
    Monitor monitor;
    ...
    monitor.setRefreshRate(
        std::round(obj.at("refreshRate").get<double>() * 100.0) / 100.0);
    monitor.setPositionX(obj.at("x").get<int>());
    monitor.setPositionY(obj.at("y").get<int>());
    monitor.setScale(obj.at("scale").get<double>());
    monitor.setTransform(
        static_cast<Transform>(obj.at("transform").get<int>()));
    ...
    log(LogLevel::Error,
        std::string("Failed to parse monitor object: ") + e.what());
    return std::nullopt;
  }
}
