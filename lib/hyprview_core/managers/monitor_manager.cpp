...
auto MonitorManager::setMonitors(std::vector<Monitor>& monitors) -> void {
    if (monitors.empty()) {
        log(LogLevel::Error, "Invalid Argument: Manager cannot manage an empty vector of monitors");
        throw std::invalid_argument("Manager cannot mange an empty vector of monitors");
    }
